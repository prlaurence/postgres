...
use constant
{
    ...
    COMMAND_EXECUTE_FUNCTION  => 'EXECUTE',
    COMMAND_GRANT             => 'GRANT',
...
################################################################################
my $strPgSqlBin = '../../../../bin/bin';    # Path of PG binaries to use for this test
...
my $bQuiet = false;                         # Supress all output except for errors
...
################################################################################
my %oCommandHash = (&COMMAND_ANALYZE => {&CLASS => &CLASS_DDL, &TYPE => &TYPE_NONE},
                    ...
                    &COMMAND_ALTER_ROLE_SET => {&CLASS => &CLASS_MISC, &TYPE => &TYPE_NONE, &COMMAND => &COMMAND_ALTER_ROLE},
                    &COMMAND_ALTER_TABLE => {&CLASS => &CLASS_DDL, &TYPE => &TYPE_TABLE},
                    &COMMAND_ALTER_TABLE_INDEX => {&CLASS => &CLASS_DDL, &TYPE => &TYPE_INDEX, &COMMAND => &COMMAND_ALTER_TABLE},
                    ...
                    &COMMAND_DROP_TABLE_INDEX => {&CLASS => &CLASS_DDL, &TYPE => &TYPE_INDEX, &COMMAND => &COMMAND_DROP_TABLE},
                    &COMMAND_DROP_TABLE_TYPE => {&CLASS => &CLASS_DDL, &TYPE => &TYPE_TYPE, &COMMAND => &COMMAND_DROP_TABLE},
                    &COMMAND_EXECUTE_FUNCTION => {&CLASS => &CLASS_FUNCTION, &TYPE => &TYPE_FUNCTION},
...
{
    capture('cd ..;make');
    CommandExecute("cp ../pgaudit.so ${strPgSqlBin}/../lib/postgresql");
    CommandExecute("cp ../pgaudit.control ${strPgSqlBin}/../share/postgresql/extension");
    CommandExecute("cp ../pgaudit--1.0.0.sql ${strPgSqlBin}/../share/postgresql/extension");
}
...
PgExecute("create extension pgaudit");
...
#
# PgLogExecute(COMMAND_DROP_TABLE, 'drop table test', 'public.test', true, false, false);
# PgLogExecute(COMMAND_DROP_TABLE_TYPE, undef, 'public.test', false, false, false);
# PgLogExecute(COMMAND_DROP_TABLE_TYPE, undef, 'public.test[]', false, true, false);
#
# PgSetUser('user2');
...
#
# # Multi-table select
# @oyTable = ({&NAME => 'public.test3', &TYPE => &TYPE_TABLE, &COMMAND => &COMMAND_SELECT},
#             {&NAME => 'public.test2', &TYPE => &TYPE_TABLE, &COMMAND => &COMMAND_SELECT});
...
#             {&NAME => 'public.test2', &TYPE => &TYPE_TABLE, &COMMAND => &COMMAND_SELECT});
# PgLogExecute(COMMAND_INSERT, 'with cte as (select id from test2) insert into test3 select id from cte',
#                              \@oyTable);
#
...
# # Reset the db before doing examples
# &log("\nReset DB:\n");
# PgDrop();
# PgCreate();
# PgStart();
...
# PgExecute("create extension pgaudit");
#
# # Now create some email friendly tests.  These first tests are session logging only.
# &log("\nExamples:");
#
# &log("\nSession Audit:\n");
#
...
PgAuditLogSet(CONTEXT_GLOBAL, undef, (CLASS_ALL, '-' . CLASS_WRITE));
PgLogExecute(COMMAND_CREATE_ROLE, 'create role audit');
